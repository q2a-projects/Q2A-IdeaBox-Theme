...
class qa_html_theme_layer extends qa_html_theme_base {
	...
	function head_css() {
		...
		if (qa_opt('qat_enable_loadingbar'))
			$this->output('<link href="' . $this->theme_url . 'includes/loader.css" type="text/css" rel="stylesheet"></link>');
	...
	function head_script()
	{
		parent::head_script();
		if (qa_opt('qat_enable_loadingbar'))
			$this->output('<script src="' . $this->theme_url .'includes/loader.js"></script>');
	...
	function doctype(){
		...
		if ( ($qa_request == 'admin/theme_options') and (qa_get_logged_in_level()>=QA_USER_LEVEL_ADMIN) ) {
			...
			$this->content['title']="IdeaBox Theme";
				
				
			$saved=false;
			// List of CSS Compression options
			...
			if (qa_clicked('qat_save_button')) {
				...
				qa_opt('qat_enable_loadingbar', (int)qa_post_text('qat_enable_loadingbar_field'));
				qa_opt('qat_show_sub_nav', (int)qa_post_text('qat_show_sub_nav_field'));
				qa_opt('qat_hide_nav', (int)qa_post_text('qat_hide_nav_field'));
			...
			// Option Form
			$options= array(
				'ok' => $saved ? 'ideabox settings saved' : null,
				'tags' => 'METHOD="POST" ACTION="'.qa_path_html(qa_request()).'"',
				'style' => 'wide', // wide , tall
				'fields' => array(
					array(
						'label' => 'Enable loading bar for each pageload',
						'type' => 'checkbox',
						'value' => qa_opt('qat_enable_loadingbar'),
						'tags' => 'NAME="qat_enable_loadingbar_field" ID="qat_enable_loadingbar_field"',
					),
					array(
						'label' => 'Hide Navigation Menu from normal users',
						'type' => 'checkbox',
						'value' => qa_opt('qat_hide_nav'),
						'tags' => 'NAME="qat_hide_nav_field" ID="qat_hide_nav_field"',
						'suffix' => 'Normal Users wont have access to Navigation Menu',
					),
					array(
						'label' => 'Show Sub-Navigation Menu in homepage',
						'type' => 'checkbox',
						'value' => qa_opt('qat_show_sub_nav'),
						'tags' => 'NAME="qat_show_sub_nav_field" ID="qat_show_sub_nav_field"',
						'suffix' => 'In homepage sub-navigation will show up to all users',
					),
